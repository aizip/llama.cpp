...
int main(int argc, char ** argv) {
    ...
    ss.clear();
    ss.str("");
    ss << basepath << fidx << ".txt";
    std::string filepath = ss.str();
    std::ifstream input_file(filepath);
    if (!input_file) {
        fprintf(stderr, "error: failed to open input_file '%d'\n", fidx);
        return -1;
    }
    // store the external input_file name in params
    // params.prompt_file = argv[i];
    params.prompt = "";
    std::copy(std::istreambuf_iterator<char>(input_file), std::istreambuf_iterator<char>(), back_inserter(params.prompt));
